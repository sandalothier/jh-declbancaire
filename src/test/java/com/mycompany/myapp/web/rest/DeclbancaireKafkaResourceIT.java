...
class DeclbancaireKafkaResourceIT {
    ...
    void setup() {
        ...
        DeclbancaireKafkaResource kafkaResource = new DeclbancaireKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/declbancaire-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/declbancaire-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
